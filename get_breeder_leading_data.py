...
from netkeiba_parser import getBreederResult
...
def main():
    ...
    parser.add_argument('--output', '-o', action='store', type=str, default='breeder_leading.csv', help='output filename')
    ...
    page = 1
    while(True):
        html = getPage("http://db.netkeiba.com/?pid=breeder_leading&year=%d&page=%d" % (year, page))
        page_result = getBreederResult(html, offset=len(results))
        results += page_result
