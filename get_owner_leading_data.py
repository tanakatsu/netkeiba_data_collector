...
from netkeiba_parser import getOwnerResult
...
def main():
    ...
    parser.add_argument('--output', '-o', action='store', type=str, default='owner_leading.csv', help='output filename')
    ...
    page = 1
    while(True):
        html = getPage("http://db.netkeiba.com/?pid=owner_leading&year=%d&page=%d" % (year, page))
        page_result = getOwnerResult(html, offset=len(results))
        results += page_result
