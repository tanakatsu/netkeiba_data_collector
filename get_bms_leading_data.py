...
from netkeiba_parser import getBmsResult
...
def main():
    ...
    parser.add_argument('--output', '-o', action='store', type=str, default='bms_leading.csv', help='output filename')
    ...
    page = 1
    while(True):
        html = getPage("http://db.netkeiba.com/?pid=bms_leading&year=%d&page=%d" % (year, page))
        page_result = getBmsResult(html, offset=len(results))
        results += page_result
